import Gift from './assets/Gift.png';
import ClientOne from './assets/client1.png';
import ClientTwo from './assets/client2.png';
import ClientThree from './assets/client3.png';
import TeamOne from './assets/team1.png';
import TeamTwo from './assets/team2.png';
import TeamThree from './assets/team3.png';
import TeamFour from './assets/team4.png';
import TeamFive from './assets/team5.png';
import TeamSix from './assets/team6.png';
import Business from './assets/BusinessLaw.png';
import Elder from './assets/Elder.png';
import Landlord from './assets/Landlord.png';
import Partnership from './assets/Partnership.png';
import RealEstate from './assets/RealEstate.png';
import Business2 from './assets/Image1.png';

export const CHOOSE_US_DETAILS = 
    {
        title: 'Why Choose Us?',
        card_details: [
            {
                id: 'choose_123',
                image: Gift,
                success_rate: '98%',
                content: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer.'
            },
            {
                id: 'choose_124',
                image: Gift,
                success_rate: '100%',
                content: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer.'
            },
            {
                id: 'choose_125',
                image: Gift,
                success_rate: '100%',
                content: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer.'
            }
        ]
    };

export const TEAM_DETAILS = {
    title: 'Our Team',
    card_details: [
        {
            id: 'team_123',
            image: TeamOne,
            name: 'Danial Def',
            no_of_cases: 301
        },
        {
            id: 'team_124',
            image: TeamTwo,
            name: 'Sanfole',
            no_of_cases: 850
        },
        {
            id: 'team_125',
            image: TeamThree,
            name: 'Cesforila',
            no_of_cases: 470
        },
        {
            id: 'team_126',
            image: TeamFour,
            name: 'Colleen',
            no_of_cases: 180
        },
        {
            id: 'team_127',
            image: TeamFive,
            name: 'Haldone',
            no_of_cases: 220
        },
        {
            id: 'team_128',
            image: TeamSix,
            name: 'Nik Jeo',
            no_of_cases: 350
        }
        
    ]
};

export const TESTIMONIALS = {
    title: 'What says are happy clients',
    card_details: [
        {
            id: 'client_123',
            image: ClientOne,
            name: 'Jane Cooper',
            destination: 'Ceo of Hunt',
            content: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer. sit aliqua dolor do amet sint. Velit officia'
        },
        {
            id: 'client_124',
            image: ClientTwo,
            name: 'Devon Lane',
            destination: 'Ceo of Hunt',
            content: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer. sit aliqua dolor do amet sint. Velit officia'
        },
        {
            id: 'client_125',
            image: ClientThree,
            name: 'Robert Fox',
            destination: 'Ceo of Hunt',
            content: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer. sit aliqua dolor do amet sint. Velit officia'
        },
        {
            id: 'client_126',
            image: ClientTwo,
            name: 'Devon Lane',
            destination: 'Ceo of Hunt',
            content: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer. sit aliqua dolor do amet sint. Velit officia'
        },
        {
            id: 'client_127',
            image: ClientThree,
            name: 'Robert Fox',
            destination: 'Ceo of Hunt',
            content: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer. sit aliqua dolor do amet sint. Velit officia'
        },
        {
            id: 'client_128',
            image: ClientOne,
            name: 'Jane Cooper',
            destination: 'Ceo of Hunt',
            content: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer. sit aliqua dolor do amet sint. Velit officia'
        },
    ]
};

export const PRACTICES = {
    title: 'Area of Practices',
    card_details: [
        {
            image: Business,
            title: 'Business Law'
        },
        {
            image: Partnership,
            title: 'Partnership Law'
        },
        {
            image: RealEstate,
            title: 'Real Estate Law'
        },
        {
            image: Business2,
            title: 'Business Law'
        },
        {
            image: Landlord,
            title: 'Landlord Disputes'
        },
        {
            image: Elder,
            title: 'Elder Abuse'
        }

    ]
};

export const QUESTIONS = [
    {
        ques: 'Do I need a personal injury report?',
        answer: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer. Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer.'
    },
    {
        ques: 'How much is my case worth?',
        answer: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer. Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer.'
    },
    {
        ques: 'What should I do right after car accidect',
        answer: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer. Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer.'
    },
    {
        ques: 'How much is my case worth?',
        answer: 'Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer. Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequatduis enim velit mollit Exer.'
    },
];